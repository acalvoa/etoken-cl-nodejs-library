...
ASN1.prototype.objectIdentifier = function(identifier) {
    const ident = identifier.replace(/\n/g, '|').split('|');
    return {
        id: ident[0],
        data: ident[1],
        type: ident[2]
...
ASN1.prototype.getAttrObject = function () {
    const content = this.content();
    if (this.typeName() === 'SEQUENCE') {
        let secuence = [];
        if (this.sub !== null) {
            for (var i = 0; i< this.sub.length; i++) {
                secuence.push(this.sub[i].getAttrObject());
            }
        }
        return secuence;
    } else if (this.typeName() === 'SET') {
        if (this.sub !== null) {
            for (var i = 0; i< this.sub.length; i++) {
                return this.sub[i].getAttrObject();
            }
        }
    } else if (this.typeName() === 'OBJECT_IDENTIFIER') {
        let element = {
            name: this.typeName(),
            value: this.objectIdentifier(content)
        };
        return element;
    } else {
        let element = {
